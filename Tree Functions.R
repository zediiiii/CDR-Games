...
cdrwincount <- function(pile,...){
  ...
  pile.length<-as.numeric(length(pile))
  element.length<-length(pile[[1]])
  win.counter<<-0
  progress.counter<<-0
  ...
  have.kids <- function(node) {
    ...
    while(pointersleft>0){
      ...
      if(child$name==endname){
        ...
        return(eval.parent(parse(text="next"),1)) #return a next function exits this loop and parent loop
      } 
      if(child$name==startname){
        win.counter<<-win.counter+1
        return(eval.parent(parse(text="next"),1)) #return a next function exits this loop and parent loop
      } 
      #Else is implied here by the return outside this loop
      progress.counter<-progress.counter+1
